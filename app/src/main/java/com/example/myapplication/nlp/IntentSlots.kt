    ...
    fun build(label: IntentLabel, command: String): AppIntent {
        ...
        return when (label) {
            ...
            IntentLabel.FOOD_ORDER -> {
                ...
                var query = EntityExtractors.targetAfter(command, listOf("order", "buy", "get"))
                    .replace("from foodgorilla", "", true)
                    .replace("from food gorilla", "", true)
                    .replace("on foodgorilla", "", true)
                    .replace("on food gorilla", "", true)
                    .trim()
                
                // Better extraction for "item from restaurant"
                val fromIndex = query.indexOf(" from ", ignoreCase = true)
                val finalQuery = if (fromIndex != -1) {
                    val item = query.substring(0, fromIndex).trim()
                    val restaurant = query.substring(fromIndex + 6).trim()
                    "$item from $restaurant"
                } else {
                    query
                }
                
                AppIntent.FoodOrder(itemId, finalQuery.takeIf { it.isNotEmpty() })
            }
